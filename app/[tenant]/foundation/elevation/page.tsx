  ...
  return (
    <div className="p-4 md:p-8 max-w-[96rem] mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-heading-lg font-bold text-ds-text mb-2">Elevation</h1>
        <p className="text-body text-ds-text-muted max-w-[60rem]">Shadow styles from Figma effect styles.</p>
      </div>
      {effectStyles.length === 0 ? (
        <div className="card p-8 flex items-start gap-4 bg-ds-bg/50">
          <div>
            <p className="text-[1.3rem] font-medium text-ds-text">No elevation styles synced yet</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
                {styles.map((style) => (
                  <div key={style.id} className="card p-6 flex flex-col gap-4">
                    ...
                    <div>
                      <div className="font-mono text-[1.2rem] text-ds-text font-medium">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-ds-border bg-ds-bg">
                      <th className="text-left px-6 py-3 text-[1.2rem] font-semibold uppercase tracking-widest text-ds-text-muted">Style</th>
                      <th className="text-left px-6 py-3 text-[1.2rem] font-semibold uppercase tracking-widest text-ds-text-muted">CSS box-shadow</th>
                    </tr>
                  </thead>
                  <tbody>
                    {styles.map((style, i) => (
                      <tr key={style.id} className={`border-b border-ds-border ${i % 2 === 1 ? 'bg-ds-bg/30' : ''}`}>
                        <td className="px-6 py-3 font-mono text-[1.3rem] text-ds-text">
                          {style.name.includes('/') ? style.name.split('/').slice(1).join('/') : style.name}
                        </td>
                        <td className="px-6 py-3 font-mono text-[1.1rem] text-ds-text-muted break-all">{style.cssBoxShadow}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
